...
class ChannelDeconv(nn.Module):
    ...
    def forward(self, x):
        print("__________________input pixel",x)
        x_shape = x.shape
        if len(x.shape)==2:
        ...
        if len(x_shape)==2:
            x1=x1.view(x_shape)
        print("__________________output pixel", x1)
...
class FastDeconv(conv._ConvNd):
    ...
    def forward(self, x):
        ...
        #print("__________________ouput pixel",x)
        self.featuremap2=x
        return x
    ...
    def get_feature_map(self):
        return self.featuremap1,self.featuremap2
